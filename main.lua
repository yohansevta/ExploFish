...
if newFishNotificationRemote then
    newFishNotificationRemote.OnClientEvent:Connect(function(fishData)
        if fishData and fishData.name and Dashboard and Dashboard.LogFishCatch then
            Dashboard.LogFishCatch(fishData.name, Dashboard.data.sessionStats.currentLocation)
...
local function DoSecureCycle()
    ...
    if miniGameRemote and miniGameRemote:IsA("RemoteFunction") then
        ...
        local timeout = 3  -- Reduced to 3 second timeout for minigame
        while tick() - minigameStart < timeout and not minigameSuccess do
            task.wait(0.05)  -- Faster polling for better responsiveness
        end
    ...
    while tick() - waitStart < 6 and not fishingCompleted do  -- Reduced maximum wait to 6 seconds
    ...
    if fishingCompleted then
        task.wait(0.1 + math.random()*0.1)  -- Faster wait for successful completion (0.1-0.2s)
    else
        print("[Secure Mode] ⚠️ Fishing may have timed out - adding recovery delay")
        task.wait(0.3 + math.random()*0.2)  -- Reduced timeout recovery (0.3-0.5s)
    end
...
local function DoFastCycle()
    ...
    if Dashboard and Dashboard.data and Dashboard.data.sessionStats then
        if not Dashboard.data.sessionStats.totalValue then
            Dashboard.data.sessionStats.totalValue = 0
        end
        ...
        Dashboard.data.sessionStats.totalValue = Dashboard.data.sessionStats.totalValue + (fishValue or 0)
        print("[Fast Mode] Simulated fish count +1, Total:", Dashboard.data.sessionStats.fishCount)
...
local function AutofishRunner(mySession)
    if Dashboard and Dashboard.data and Dashboard.data.sessionStats then
        Dashboard.data.sessionStats.startTime = tick()
        Dashboard.data.sessionStats.fishCount = 0
        Dashboard.data.sessionStats.rareCount = 0
        Dashboard.data.sessionStats.totalValue = 0  -- Initialize totalValue
    end
...
local function BuildUI()
    ...
    local function UpdateDashboard()
        ...
        if not Dashboard or not Dashboard.data or not Dashboard.data.sessionStats then
            sessionFishCount.Text = "🎣 Total Fish: 0"
            sessionRareCount.Text = "✨ Rare Fish: 0"
            sessionTime.Text = "⏱️ Session: 0m 0s"
            sessionLocation.Text = "🗺️ Location: Unknown"
        ...
        print("[Dashboard] Updating stats - Fish:", Dashboard.data.sessionStats.fishCount, "Rare:", Dashboard.data.sessionStats.rareCount)
        ...
        sessionFishCount.Text = "🎣 Total Fish: " .. Dashboard.data.sessionStats.fishCount
        sessionRareCount.Text = "✨ Rare Fish: " .. Dashboard.data.sessionStats.rareCount
        sessionTime.Text = string.format("⏱️ Session: %dm %ds", minutes, seconds)
        sessionLocation.Text = "🗺️ Location: " .. Dashboard.data.sessionStats.currentLocation
        
        -- Calculate efficiency
        local rareRate = Dashboard.data.sessionStats.fishCount > 0 and 
                        math.floor((Dashboard.data.sessionStats.rareCount / Dashboard.data.sessionStats.fishCount) * 100) or 0
        local fishPerMin = sessionDuration > 0 and (Dashboard.data.sessionStats.fishCount / (sessionDuration / 60)) or 0
    ...
    for islandName, cframe in pairs(islandLocations) do
        ...
        for _, btn in pairs(buttons) do
            if btn.Text == islandName then
                ...
                btn.MouseButton1Click:Connect(function()
                    if Dashboard and Dashboard.data and Dashboard.data.sessionStats then
                        Dashboard.data.sessionStats.currentLocation = islandName:gsub("🏝️", ""):gsub("🦈 ", ""):gsub("🎣 ", ""):gsub("❄️ ", ""):gsub("🌋 ", ""):gsub("🌴 ", ""):gsub("🗿 ", ""):gsub("⚙️ ", "")
                    end
                end)
                break
            end
...
-- Setup Dashboard with fish event listener
if Dashboard and Dashboard.SetupFishCaughtListener then
    Dashboard.SetupFishCaughtListener(fishCaughtRemote)
else
    print("[ModuleLoader] ⚠️ Dashboard module not available - using fallback")
end

-- Start location tracker (compatibility mode)
local function LocationTracker()
    while true do
        if Dashboard and Dashboard.DetectCurrentLocation then
            local newLocation = Dashboard.DetectCurrentLocation()
            if Dashboard.data and Dashboard.data.sessionStats and 
               newLocation ~= Dashboard.data.sessionStats.currentLocation then
...
_G.ModernAutoFish = {
    ...
    -- Dashboard API
    LogFish = function(fishName, location)
        if Dashboard and Dashboard.LogFishCatch then
            Dashboard.LogFishCatch(fishName, location)
        end
    end,
    GetStats = function() return Dashboard and Dashboard.data or {} end,
    ClearStats = function() 
        if Dashboard and Dashboard.ResetAllData then
            Dashboard.ResetAllData()
        end
    end,
    SetLocation = function(loc) 
        if Dashboard and Dashboard.data and Dashboard.data.sessionStats then
            Dashboard.data.sessionStats.currentLocation = loc 
        end
    end,
